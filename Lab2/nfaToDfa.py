...
for i in range(n):
    state = input("state name : ")  # Enter state name eg: A, B, C or 0, 1, 2 ..etc
    nfa[state] = {}  # Creating a nested dictionary
    for j in range(t):
        path = input("path : ")  # Enter path:
...
new_states_list = []
dfa = {}  # dfa dictionary
keys_list = list(list(nfa.keys())[0])  # contains all the states in nfa plus the states created in dfa are also appended further
path_list = list(nfa[keys_list[0]].keys())  # list of all the paths
